...
func main() {
	...
	agentDeploymentWebhook := agent.NewAgentDeploymentWebhook(&agent.AgentDeploymentWebhookConfig{
		FIPSEnabled: fipsEnabled,
	})
	if err = agentDeploymentWebhook.SetupWebhookWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create webhook", "webhook", "Pod")
		os.Exit(1)
	}
