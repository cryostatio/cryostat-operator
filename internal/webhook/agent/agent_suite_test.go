...
var agentWebhookConfig *agent.AgentWebhookConfig
...
var _ = BeforeSuite(func() {
	...
	err = webhook.SetupWebhookWithManager(mgr, &operatorv1beta2.Cryostat{})
	Expect(err).NotTo(HaveOccurred())

	agentWebhookConfig = &agent.AgentWebhookConfig{
		OSUtils: test.NewTestOSUtils(&test.TestReconcilerConfig{}),
	}
	agentWebhook := agent.NewAgentWebhook(agentWebhookConfig)
